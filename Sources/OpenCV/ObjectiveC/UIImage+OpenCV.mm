...
- (id)initWithCVMat:(const cv::Mat&)cvMat {
    ...
    CGBitmapInfo bitmatInfo;
    
    if (cvMat.elemSize() == 1) {
        colorSpace = CGColorSpaceCreateDeviceGray();
        bitmatInfo = kCGImageAlphaNone|kCGBitmapByteOrderDefault;
    } else {
        colorSpace = CGColorSpaceCreateDeviceRGB();
        bitmatInfo = kCGImageAlphaNoneSkipLast|kCGBitmapByteOrderDefault;
    }
